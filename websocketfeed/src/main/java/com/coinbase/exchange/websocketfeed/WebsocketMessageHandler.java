...
public class WebsocketMessageHandler implements WebsocketFeed.MessageHandler {
    ...
    public void handleMessage(final String json) {
        CompletableFuture.runAsync(() -> {
            ...
            } else if (message instanceof TickerMessage) {
                handleTicker((TickerMessage) message);
            }
    ...
    protected void handleError(ErrorOrderBookMessage message, String json) {
    ...
    protected void handleHeartbeat(HeartBeat message) {
        HeartBeat heartBeat = message;
        log.info("HEARTBEAT. Last trade id: {}", heartBeat.getLast_trade_id());
    }

    protected void handleTicker(TickerMessage message) {
        TickerMessage tickerMessage = message;
